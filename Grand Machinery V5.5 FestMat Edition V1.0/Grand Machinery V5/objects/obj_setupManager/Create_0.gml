...
#region Input manager
	...
	with (InputManager) {
		...
		array[input_ID.primaryUse].controller = debugCreateControllerStruct(gp_face1, false, 1, false, false);
		// TODO - Support for secondary use here.
		...
		array[input_ID.interact].controller = debugCreateControllerStruct(gp_axislv, true, -1, false);
		array[input_ID.down].controller = debugCreateControllerStruct(gp_axislv, true, 1, false, false);
		array[input_ID.jump].controller = debugCreateControllerStruct(gp_face2, false, 1, false, false);
		...
		array[input_ID.hlthPacketContinuous].controller = debugCreateControllerStruct(gp_face3, false, 1, false, false);
		array[input_ID.hlthPacket].controller = debugCreateControllerStruct(gp_face3, false, 1, true, false);
...
global.stageManager = {
	...
	setStageSize: function(stageIDI, widthI, heightI) {
		...
		TileManager.assignTileSetToGameplayBlueprint(curStage.initialGameplayBlueprint, widthI, heightI);
	},
...
#macro TileManager global.gameplayManager.tileManager
	...
	#macro type_defaultType_tileAmount 13 // OBSERVATION001 - Rename this variable.
	enum type_defaultType { // OBSERVATION001 - Rename this variable.
		outerCornerLeftUp, outerCornerRightUp, borderUp, borderLeft,
		outerCornerLeftDown, outerCornerRightDown, borderDown, borderRight,
		innerCornerLeftUp, innerCornerRightUp, center,
		innerCornerLeftDown = 11, innerCornerRightDown = 12
	}
...
global.gameplayManager = {
	...
	initializeTools: function() {
		...
		with (toolArray[tool_ID.dagger]) {
			tick = function() {
				if (InputManager.isInputActivated(input_ID.primaryUse) && !PlayerManager.isDead) {
					obj_player.slash.start(spr_daggerSlash, 2, 6, spr_daggerSlashCollision, 1, 18);
				}
			}
		}
		...
		with (toolArray[tool_ID.gun]) {
			tick = function() {
				if (InputManager.isInputActivated(input_ID.secondaryUse) && !PlayerManager.isDead) {
	...
	pointManager: {
		...
		notifyPlayerDmg: function(dmgI) {
			...
			if (isComboActive) {
				comboDecreaseLifetime(comboLifetime/2);
			}
		},
		#endregion
	...
	cameraManager: {
		...
		beginStepEvent: function() {
			...
			camera_set_view_pos(view_camera[0], curX, curY);
		}
	...
	stageObjectManager: {
		...
		initialize: function() {
			...
			#region DECORATION
				...
				#region obj_transitionDemarkingBase
				
				type_transitionDemarkingBase = constructStageObjType(obj_transitionDemarkingBase, layers.collision);
				type_transitionDemarkingBase.hasPosition = true;
				type_transitionDemarkingBase.hasScaling = true;
				type_transitionDemarkingBase.add = function(xI, yI, xScaleI, yScaleI) {
					with (StageObjectManager) {
						var newStageObj = constructBase(type_transitionDemarkingBase);
						addPosition(newStageObj, xI, yI);
				...
				#region obj_transitionDemarkingEdge
				
				type_transitionDemarkingEdge = constructStageObjType(obj_transitionDemarkingEdge, layers.collision);
				type_transitionDemarkingEdge.hasPosition = true;
				type_transitionDemarkingEdge.hasScaling = true;
				type_transitionDemarkingEdge.hasAngle = true;
				type_transitionDemarkingEdge.add = function(xI, yI, xScaleI, yScaleI, angleI) {
					with (StageObjectManager) {
						var newStageObj = constructBase(type_transitionDemarkingEdge);
						addPosition(newStageObj, xI, yI);
						addScaling(newStageObj, xScaleI, yScaleI);
			...
			#region PROPS
				...
				type_energyCollision.add = function(xI, yI, xScaleI, yScaleI, energyUseFrameI) {
					with (StageObjectManager) {
						...
						addPosition(newStageObj, xI, yI);
						addScaling(newStageObj, xScaleI, yScaleI);
						addEnergyInterface(newStageObj, 1, 1);
				...
				if (actionObjI.frame >= actionObjI.frameMin && actionObjI.frame < actionObjI.frameMax) {
					...
					for (var i = 0; i < gridCollisionLength; i++) {
						...
						if (
							...
							if (actionObjI.isSingleTarget) actionObjI.hasHit = true;
	...
	initializeInterfaces: function() { // OBSERVATION095 - Change all interface instance variable by if[respective interface]
		...
		physicsInterface.tickAll = function() {
			...
			for (var i = 0; i < array_length(curInterfaces); i++) {
				if_physics.tick(curInterfaces[i]);
			}
		}
	...
	tileManager: {
		tileMapSpr: undefined,
		
		#region EVENTS
		stageStartEvent: function() {
			tileMapSpr = createTileMapSprFromTileArr(StageManager.currentStage.initialGameplayBlueprint.tileArr);
		},
		stageEndEvent: function() {
			sprite_delete(tileMapSpr);
		},
		drawEndEvent: function() {
			drawTileMapSpr(tileMapSpr);
		},
		#endregion
		
		#region Tile map creation
		curTileArr: undefined,
		assignTileSetToGameplayBlueprint: function(gameplayBlueprintI, roomWidthI, roomHeightI) {
			var curW = ceil(roomWidthI/8);
			var curH = ceil(roomHeightI/8);
			
			var tileArr = array_create(curW);
			for (var i = 0; i < curW; i++) {
				tileArr[i] = array_create(curH, undefined);
			}
			gameplayBlueprintI.tileArr = tileArr;
		},
		setTileTypesByInstance: function(instI, tileSetTypeI) {
			var x1 = instI.x/8;
			var y1 = instI.y/8;
			var x2 = (instI.x+instI.sprite_width)/8;
			var y2 = (instI.y+instI.sprite_height)/8;
			for (var i = x1; i < x2; i++) {
				for (var j = y1; j < y2; j++) {
					curTileArr[i][j] = tileSetTypeI;
				}
			}
		},
		#endregion
		...
		#region Loading
		createTileMapSprFromTileArr: function(tileArrI) {
			var surfWidth = array_length(tileArrI)*8;
			var surfHeight = array_length(tileArrI[0])*8;
			var sprSurface = surface_create(surfWidth, surfHeight);
			
			surface_set_target(sprSurface);
			...
			for (var i = 0; i < array_length(tileArrI); i++) {
				...
				for (var j = 0; j < array_length(curRow); j++) {
					...
					if (curTile != undefined) {
						curTile.type.drawTile(curTile, tileArrI, i, j);
					}
				}
			}
			...
			var newTileMapSpr = sprite_create_from_surface(sprSurface, 0, 0, surfWidth, surfHeight, false, false, 0, 0);
			surface_free(sprSurface);
			return newTileMapSpr;
		},
		drawTileMapSpr: function(tileMapSprI) {
			draw_sprite(tileMapSprI, 0, 0, 0);
		},
		#endregion
		
		#region Types
		tileSet_metal1: undefined,
		initializeTileSets: function() {
			tileSet_metal1 = type_default.construct(tileSpr_metal1);
		},
		
		type_default: {
			construct: function(baseSprI) {
				var sprWidth = sprite_get_width(baseSprI);
				var sprHeight = sprite_get_height(baseSprI);
				
				var tileSetSpr = undefined;
				var sprSurf = surface_create(sprWidth, sprHeight);
				
				surface_set_target(sprSurf);
				draw_sprite(baseSprI, 0, 0, 0);
				surface_reset_target();
				
				var tileXPosArr = [0, 8, 16, 24  ,  0, 8, 16, 24  ,  0, 8, 16  ,  0, 8];
				var tileYPosArr = [0, 0, 0, 0  ,  8, 8, 8, 8  ,  16, 16, 16  ,  24, 24];
				for (var i = 0; i < type_defaultType_tileAmount; i++) {
					if (tileSetSpr == undefined) tileSetSpr = sprite_create_from_surface(
						sprSurf, tileXPosArr[i], tileYPosArr[i], 8, 8,
						false, false, 0, 0
					);
					else sprite_add_from_surface(
						tileSetSpr, sprSurf, tileXPosArr[i], tileYPosArr[i], 8, 8,
						false, false
					);
				}
			...
			drawTile: function(tileSetI, tileArrI, xI, yI) {
				var w = array_length(tileArrI);
				var h = array_length(tileArrI[0]);
				if (xI%2 == 0) {
					if (yI%2 == 0) { // Top left corner
						var isCollidingLeft = xI-1 < 0;
						var isCollidingUp = yI-1 < 0;
						if (!isCollidingLeft && tileArrI[xI-1][yI] == undefined) {
							if (!isCollidingUp && tileArrI[xI][yI-1] == undefined)
								draw_sprite(tileSetI.tileSprites, type_defaultType.outerCornerLeftUp, xI*8, yI*8);
							else draw_sprite(tileSetI.tileSprites, type_defaultType.borderLeft, xI*8, yI*8);
						}else {
							if (!isCollidingUp && tileArrI[xI][yI-1] == undefined) {
								draw_sprite(tileSetI.tileSprites, type_defaultType.borderUp, xI*8, yI*8);
							}else {
								if (!isCollidingUp && !isCollidingLeft && tileArrI[xI-1][yI-1] == undefined)
									draw_sprite(tileSetI.tileSprites, type_defaultType.innerCornerLeftUp, xI*8, yI*8);
								else draw_sprite(tileSetI.tileSprites, type_defaultType.center, xI*8, yI*8);
							}
						}
					}else {  // Bottom left corner
						var isCollidingLeft = xI-1 < 0;
						var isCollidingBottom = yI+1 >= h;
						if (!isCollidingLeft && tileArrI[xI-1][yI] == undefined) {
							if (!isCollidingBottom && tileArrI[xI][yI+1] == undefined)
								draw_sprite(tileSetI.tileSprites, type_defaultType.outerCornerLeftDown, xI*8, yI*8);
							else draw_sprite(tileSetI.tileSprites, type_defaultType.borderLeft, xI*8, yI*8);
						}else {
							if (!isCollidingBottom && tileArrI[xI][yI+1] == undefined) {
								draw_sprite(tileSetI.tileSprites, type_defaultType.borderDown, xI*8, yI*8);
							}else {
								if (!isCollidingBottom && !isCollidingLeft && tileArrI[xI-1][yI+1] == undefined)
									draw_sprite(tileSetI.tileSprites, type_defaultType.innerCornerLeftDown, xI*8, yI*8);
								else draw_sprite(tileSetI.tileSprites, type_defaultType.center, xI*8, yI*8);
							}
						}
					}
				}else {
					if (yI%2 == 0) { // Top right corner
						var isCollidingRight = xI+1 >= w;
						var isCollidingUp = yI-1 < 0;
						if (!isCollidingRight && tileArrI[xI+1][yI] == undefined) {
							if (!isCollidingUp && tileArrI[xI][yI-1] == undefined)
								draw_sprite(tileSetI.tileSprites, type_defaultType.outerCornerRightUp, xI*8, yI*8);
							else draw_sprite(tileSetI.tileSprites, type_defaultType.borderRight, xI*8, yI*8);
						}else {
							if (!isCollidingUp && tileArrI[xI][yI-1] == undefined) {
								draw_sprite(tileSetI.tileSprites, type_defaultType.borderUp, xI*8, yI*8);
							}else {
								if (!isCollidingUp && !isCollidingRight && tileArrI[xI-1][yI-1] == undefined)
									draw_sprite(tileSetI.tileSprites, type_defaultType.innerCornerRightUp, xI*8, yI*8);
								else draw_sprite(tileSetI.tileSprites, type_defaultType.center, xI*8, yI*8);
							}
						}
					}else {  // Bottom right corner
						var isCollidingRight = xI+1 >= w;
						var isCollidingDown = yI+1 >= h;
						if (!isCollidingRight && tileArrI[xI+1][yI] == undefined) {
							if (!isCollidingDown && tileArrI[xI][yI+1] == undefined)
								draw_sprite(tileSetI.tileSprites, type_defaultType.outerCornerRightDown, xI*8, yI*8);
							else draw_sprite(tileSetI.tileSprites, type_defaultType.borderRight, xI*8, yI*8);
						}else {
							if (!isCollidingDown && tileArrI[xI][yI+1] == undefined) {
								draw_sprite(tileSetI.tileSprites, type_defaultType.borderDown, xI*8, yI*8);
							}else {
								if (!isCollidingDown && !isCollidingRight && tileArrI[xI-1][yI+1] == undefined)
									draw_sprite(tileSetI.tileSprites, type_defaultType.innerCornerRightDown, xI*8, yI*8);
								else draw_sprite(tileSetI.tileSprites, type_defaultType.center, xI*8, yI*8);
							}
						}
					}
		...
		type_verticalCyclic: {
			construct: function(baseSprI, cycleLengthI) {
...
global.stageBuilderFromRoom = {
	...
	roomStartEvent: function() {
		if (isBuilding) {
			...
			TileManager.curTileArr = curStage.initialGameplayBlueprint.tileArr;
